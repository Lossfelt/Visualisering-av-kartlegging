...
labelRenderer.domElement.style.pointerEvents = 'none'; // Ensures controls remain functional
...
function createLabel(text, position, axis, index) {
    ...
    div.style.pointerEvents = 'auto';  // Gjør labels klikkbare
...
for (let x = 0; x < clinics; x++) {
    createLabel(`Clinic ${x + 1}`, new THREE.Vector3((x - clinics / 2) * spacing, (0 - labAreas / 2) * spacing, -processes / 2 * spacing - cubeSize), 'x', x);
}
for (let y = 0; y < labAreas; y++) {
    createLabel(`Lab ${y + 1}`, new THREE.Vector3(-clinics / 2 * spacing - cubeSize, (y - labAreas / 2) * spacing, (0 - processes / 2) * spacing), 'y', y);
}
for (let z = 0; z < processes; z++) {
    createLabel(`Process ${z + 1}`, new THREE.Vector3((0 - clinics / 2) * spacing, -labAreas / 2 * spacing - cubeSize, (z - processes / 2) * spacing), 'z', z);
}
